  ...
  return (
    <div>
      ...
      <button style={{marginBottom: '30px'}}>
        <Link className="btn btn-success bg-success text-white border-white" to="/admin/addCategory">Thêm danh mục</Link>
      </button>
...

          {
            categories.map((item, index) => {
              ...
              <td>
                ...
                to={`/admin/editCategory/${item.id}`}>Sửa</ Link></button>
                <button><Link className="btn btn-info bg-danger text-white border-white"
                onClick={() => delCategory(item.id)} >Xóa</ Link></button>
              </td>
              </tr>
              );
            })              
